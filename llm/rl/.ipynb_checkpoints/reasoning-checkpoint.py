...
class ConversationalReasoner:
    """
    An object-oriented engine for System 2 thinking and self-refinement 
    in general conversational text generation.
    """
    def __init__(self, inference_engine):
        # Maps directly to your Qwen3InferenceEngine
        self.engine = inference_engine

    def _generate_silent(self, prompt, max_new_tokens, temperature, top_p):
        """Silently generates text using the underlying inference engine."""
        full_text = ""
    ...
    def refine(self, user_input, iterations=1, max_response_tokens=512, max_critique_tokens=150, verbose=False, temperature=0.7, top_p=0.9):
        """
        Executes the self-refinement loop to draft, critique, and improve responses.
        """
        steps = []
        ...
        for it in range(iterations):
            ...
            # Assume the edited version is better for general chat
...
class ReasoningMetrics:
    """
    Utility class to calculate log-probabilities and evaluate 
    sequence joint-probabilities directly from the model's logits.
    """
    def __init__(self, inference_engine):
        self.engine = inference_engine
...
if __name__ == "__main__":
    ...
    # 2. Instantiate our new Object-Oriented Handlers
    agent = ConversationalReasoner(base_engine)
    metrics = ReasoningMetrics(base_engine)
    
    # 3. Test the Self-Refinement Engine
